...
export default function TabbarExercise() {
  ...
  const exerciseSchema = z.object({
    ...
    muscle: z
      ...
      .min(1, "Vui lòng chọn ít nhất 1 nhóm cơ"),
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => {
            (setOpenForm(true), setTitle("Thêm bài tập"), handleReset());
          }}
      ...
      <div className="flex gap-3 mb-6">
        <button
          onClick={() => {
            (fetchAllExercise(), setActiveTab("Tất cả"));
          }}
          className={`px-4 py-2 rounded-full text-sm border ${activeTab === "Tất cả" ? "bg-purple-600 text white" : "bg-white text-gray-600"}`}
        >
          Tất cả
        </button>
        {listMuscle.map((m, i) => (
          <button
            onClick={() => {
              (fetchExerciseByMuscleGroupId(m.id), setActiveTab(m.name));
            }}
            key={i}
