...
export default function UpgradePaymentPage() {
  ...
  let pageConfig = {
    ...
    alertContent: (
      <span>
        Gói hiện tại của bạn sẽ kết thúc.{" "}
        <span className="text-purple-400 font-bold">
          Giá trị sử dụng còn lại
        </span>{" "}
        của gói cũ sẽ được hệ thống tính toán và{" "}
        <span className="text-purple-400 font-bold">
          trừ trực tiếp vào hóa đơn
        </span>{" "}
        thanh toán của gói mới.
      </span>
    ),
  };
  ...
  if (isNewPurchase) {
    pageConfig = {
      ...
      alertContent: (
        <span>
          Gói tập của bạn sẽ được kích hoạt và tính ngày{" "}
    ...
    pageConfig = {
      ...
      alertContent: (
        <span>
          Thời gian của gói mới sẽ được{" "}
          <span className="text-green-400 font-bold">cộng dồn</span> tiếp vào
          ngày hết hạn hiện tại của bạn.
        </span>
      ),
    };
  }
    ...
    <div className="min-h-screen relative flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-lg transition-all duration-300">
        ...
        <div className="bg-gray-900/80 backdrop-blur-sm border-2 border-yellow-400 rounded-2xl px-6 py-8 shadow-2xl">
          <div
            className={`mb-6 border rounded-xl p-4 flex items-start gap-3 ${pageConfig.alertBg}`}
          >
            {pageConfig.alertIcon}
            <div className="text-sm text-left">
              <p
                className={`font-bold mb-1 uppercase text-xs tracking-wide ${pageConfig.alertTitleColor}`}
              >
