...
export default function MemberRegisterPage() {
  ...
  const DAYS = Array.from({ length: 7 }).map((_, i) => {
    ...
    return {
      date,
      labelDay: format(date, "EEEE", { locale: vi }),
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-[32px] border border-slate-200 shadow-xl p-8 md:p-10 text-center">
            ...
            <p className="text-slate-500 font-medium leading-7 max-w-2xl mx-auto">
              Hiện tại tài khoản của bạn chưa có huấn luyện viên phụ trách. Vui
              lòng chờ Admin chọn huấn luyện viên cho bạn trước khi đăng ký lịch
              tập.
            </p>

            <div className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-700 font-medium">
              Khi Admin phân công PT xong, bạn có thể quay lại trang này để xem
              lịch và đăng ký buổi tập.
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
              ...
              <p className="text-slate-500 font-medium mt-1">
                Đăng ký các khung giờ trống để huấn luyện viên hướng dẫn bạn.
              </p>
            </div>
            {/* DATE NAVIGATION */}
            ...
            <div className="flex items-center bg-white p-2 rounded-2xl shadow-xl shadow-slate-200/40 border border-slate-200 self-start md:self-center">
              ...
              <div className="px-6 font-black text-slate-900 text-sm min-w-[180px] text-center uppercase tracking-tighter">
                ...
                {format(addDays(weekStart, 6), "dd MMM", { locale: vi })}
              </div>
              <button
                onClick={() => setWeekStart(addDays(weekStart, 7))}
                className="p-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl transition-all active:scale-95"
            ...
            <div className="grid grid-cols-[80px_repeat(7,1fr)] overflow-x-auto min-w-[900px]">
              ...
              {DAYS.map((d, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`text-center py-6 border-b border-r last:border-r-0 border-slate-100 ${isToday ? "bg-purple-50/30" : ""}`}
                  >
                    <div
                      className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${isToday ? "text-purple-600" : "text-slate-400"}`}
                    >
                      {d.labelDay.replace("Thứ ", "T")}
                    </div>
                    <div
                      className={`text-2xl font-black ${isToday ? "text-purple-600" : "text-slate-900"}`}
                    >
                      {format(d.date, "dd")}
                    </div>
                  </div>
              ...
              <div className="bg-slate-50/30 border-r border-slate-100">
                {HOURS.map((h) => (
                  <div
                    ...
                    className="flex items-start justify-center pt-3 text-[11px] font-bold text-slate-400 tabular-nums"
                  >
                    {h}:00
                  </div>
                ))}
                    ...
                    .map((s) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() =>
                            !isBooked && !isPast && handleRegister(s)
                          }
                          className={`absolute z-10 p-3 rounded-2xl flex flex-col justify-between transition-all duration-300 group/item overflow-hidden
                          ${
                            isBooked || isPast
                        ...
                        >
                          <div className="flex items-center justify-between relative z-10">
                            <span
                              className={`text-[10px] font-black tabular-nums ${isBooked ? "text-slate-400" : "text-emerald-500 group-hover/item:text-emerald-100"}`}
                            >
                              {s.start_time}
                            </span>
                            {isBooked || isPast ? (
                              ...
                              <Zap className="w-3 h-3 opacity-50 fill-current" />
                            )}
                          </div>
          ...
          <div className="mt-10 p-6 rounded-[24px] bg-slate-100 border border-slate-200 flex items-start gap-4">
            ...
            <div className="text-sm text-slate-500 leading-relaxed font-medium">
              <strong>Ghi chú đăng ký:</strong> Lịch tập sẽ được chốt sau khi PT
              xác nhận. Nếu bạn cần dời lịch hoặc hủy buổi tập, vui lòng thông
              báo cho PT trước ít nhất 2 giờ để đảm bảo quyền lợi và sắp xếp
              khung giờ mới.
            </div>
          </div>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <div className="relative bg-white rounded-[32px] p-8 w-full max-w-md shadow-2xl border border-slate-200 animate-in zoom-in duration-200">
                ...
                <p className="text-slate-500 font-medium mb-8 leading-relaxed">
                  Bạn đang thực hiện đăng ký lịch tập cùng huấn luyện viên{" "}
                  <span className="text-slate-900 font-bold">{myPT.name}</span>{" "}
                  vào lúc:
                  <span className="block mt-2 px-4 py-3 bg-slate-50 rounded-2xl text-purple-600 font-black text-lg">
                    {format(parseISO(selectedSchedule.date), "EEEE, dd/MM", {
                      locale: vi,
                    })}
