...
export default function SideBar({collapsed, setCollapsed}) {
    return(
        <div className={`fixed top-0 left-0 h-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white transition-all duration-300 ${collapsed ? "w-24" : "w-64"}`}>
            ...
            <nav className="mt-4 border-t border-gray-300 px-2">
                <ul className="mt-2 space-y-5">
                  <NavLink 
                        ...
                        <span className={`${collapsed ? "hidden" : "inline"} max-sm:hidden text-lg`}>Dashboard</span>
                  </NavLink>
                  <NavLink
                        to="/admin/user"
                        className={({ isActive }) =>
