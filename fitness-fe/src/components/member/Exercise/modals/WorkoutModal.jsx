...
export default function WorkoutModal({ exercises = [], open, onClose, workoutToday, workoutId, reloadWorkout }) {
    ...
    const [timeLeft, setTimeLeft] = useState(localExercises[0]?.execution_time || 0);
            ...
            <div className="w-full rounded-3xl flex">
                ...
                <div className="w-1/4 border-r border-gray-700 p-4 space-y-3 h-[80vh] overflow-y-auto custom-scroll">
                    ...
                    {localExercises.map((ex, i) => (
                        <div key={ex.id} onClick={() => selectExercise(i)} className={`flex items-center p-3 rounded-xl border ${i === currentIndex ? "bg-purple-600/30 border-purple-500" : "border-gray-700"}`}>
                ...
                <div className="w-2/4 flex flex-col items-center justify-center gap-6">
                    ...
                    <div className="flex gap-10">
                        <div className="text-center">
                            <p className="text-gray-400 text-sm">Thời gian buổi tập hiện tại</p>
                ...
                <div className="w-2/4 border-l border-gray-700 p-4 space-y-4">
                    <div className="space-y-2 text-sm text-gray-300">
                        <div className="w-100 h-55">
                            <iframe
                                ...
                                src={currentExercise?.exercise?.video?.replace("watch?v=", "embed/")}
                                title="Exercise Video"
                                allowFullScreen
                            />
                        </div>
                        <h3 className="font-bold text-xl text-center">{currentExercise?.exercise?.name}</h3>
                        <div className="flex justify-center gap-6">
                            <div className="flex flex-col items-center p-2">
                                <span>Sets</span>
                                <span className="font-bold text-purple-500">{currentExercise?.set_count}</span>
                            </div>
                            <div className="flex flex-col items-center p-2">
                                <span>Reps</span>
                                <span className="font-bold text-purple-500">{currentExercise?.rep}</span>
                            </div>
                            <div className="flex flex-col items-center p-2">
                                <span>Thời gian</span>
                                <span className="font-bold text-purple-500">{currentExercise?.execution_time}s</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-center">
                        <button onClick={() => setShowConfirm(true)} className="font-xl bg-red-500 cursor-pointer rounded-md px-4 py-2 mt-5 text-white font-bold">Kết thúc buổi tập</button>
                    </div>
                </div>
            </div>
            <ConfirmModal open={showConfirm} onClose={() => setShowConfirm(false)} onConfirm={finishWorkout}
