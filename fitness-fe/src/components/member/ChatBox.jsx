...
const BotAvatarPng = ({ className = "w-12 h-12" }) => (
  <div className={`${className} rounded-full overflow-hidden bg-purple-100 border-2 border-purple-200 flex items-center justify-center shrink-0 shadow-sm`}>
...
export default function ChatBox() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, text: "Xin chào! 👋 Cảm ơn bạn đã quan tâm đến phòng tập của chúng tôi. Bạn cần tư vấn về gói tập hay dịch vụ nào ạ?", sender: "bot" }
  ]);
        ...
        <div className="w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col h-[600px] animate-in slide-in-from-bottom-5 fade-in duration-300">
          ...
          <div className="bg-gradient-to-r from-purple-700 to-purple-500 p-4 text-white flex justify-between items-center shadow-md z-10 shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-lg text-white m-0">Hỗ trợ trực tuyến</h3>
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col gap-4 scrollbar-thin scrollbar-thumb-gray-300">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex items-end gap-2 max-w-[85%] ${msg.sender === "user" ? "self-end flex-row-reverse" : ""}`}>
                ...
                <div className={`p-3.5 rounded-2xl text-[15px] shadow-sm ${
                  msg.sender === "user" 
